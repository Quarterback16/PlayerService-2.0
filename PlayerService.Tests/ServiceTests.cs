...
namespace PlayerService.Tests
{
    ...
    public class ServiceTests
    {
        ...
        [TestMethod]
        public void PlayerService_CanRetrieveByName()
        {
            var p = _cut?
                .Search(
                    (NflPlayerState p) => p.Name == "Kenneth Gainwell")
                .FirstOrDefault();
            Assert.IsNotNull(p);
            Console.WriteLine($"Plyr is {p.Name} and team is {p.CurrTeam}");
        }

        [TestMethod]
        public void PlayerService_CanGetByName()
        {
            var p = _cut?
                .GetByName("Kenneth Gainwell");
            Assert.IsNotNull(p);
            Console.WriteLine($"Plyr is {p.Name} and team is {p.CurrTeam}");
        }
